...
Claude 내장 **WebFetch** 대비 실측 비교입니다. WebFetch 토큰 = `web_fetch_20260209` server tool 실행 후 LLM 컨텍스트 윈도우에 실제로 들어오는 `input_tokens`. DomPruner 토큰 = DOM AST 정제 후 `refined_tokens`.

`†` = BM25 점수 0 폴백: 쿼리 용어 미매칭 → 전체 클린 콘텐츠 자동 반환.

| 사이트 | WebFetch (실측) | **DomPruner** | **절감률** | 모드 |
|--------|:---------------:|:-------------:|:----------:|:----:|
...
DomPruner는 WebFetch 대비 평균 **93.5% 적은 컨텍스트 토큰**으로 — 요약 모델 없이 원문 구조 그대로 전달합니다.

### vs WebFetch

| | WebFetch (내장) | **DomPruner** |
|---|:---:|:---:|
| 평균 컨텍스트 토큰 | ~15,735 | **~1,019 (93.5% 절감)** |
| 콘텐츠 충실도 | 소형 모델 요약 | **원문 그대로 보존** |
| 요약 손실 | 있음 | **없음** |
| 추가 API 키 | 불필요 | **불필요** |
| 레이턴시 | Fetch + 모델 추론 | **Fetch + 파싱 (~300ms)** |
