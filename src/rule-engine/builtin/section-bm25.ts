 ...
 *
 * - 토큰 예산 기반 선택 (기본 1,200 tok): LLM 툴 콜 1회당 컨텍스트 할당량.
 *   섹션 수 제한이 아닌 토큰 수 제한 — 섹션 크기가 페이지마다 다르기 때문에
 *   "몇 개" 기준은 실제 컨텍스트 소비와 무관해진다.
...
export interface SectionBm25Options {
   ...
   *
   * LLM 툴 콜 1회당 컨텍스트 할당량으로 설계됨.
   * 웹 페이지 섹션은 크기가 수십~수천 tok으로 다양하므로 "섹션 수"가 아닌
   * "토큰 수"로 제한해야 실제 컨텍스트 소비가 예측 가능해진다.
   * - 1,200 tok ≈ 900 단어 — 팩트 쿼리 답변에 충분한 양
   * - minSections=2 로 최소 2개는 항상 보장
   * - TOC(120 tok)는 별도 가산 → 실질 상한 ~1,320 tok
   */
  tokenBudget?: number;
  /** 헤딩 필드 가중치 배율 (기본: 2.5) */
  headingBoost?: number;
