...
All notable changes to dompruner-mcp are documented here.

## [0.1.0] — 2026-08-06

### Added

- `dompruner_fetch(url, query?)` — fetch any URL and return DOM-refined Markdown with 90%+ token reduction
- `dompruner_analyze(url)` — token reduction report + render type + Semantic Anchor list
- `dompruner_workflow` prompt — injected on MCP connection to set correct usage pattern in the host LLM
